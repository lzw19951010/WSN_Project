...
class Data {
    ...
    void update(int nodeId, int messageId, int TempData, int HumidityData, int PhotoData) {
    ...
    Node node1 = nodes[nodeId-100];
    Node node2 = nodes[nodeId-120];
    if (node == null) {
        nodes[nodeId] = node = new Node(nodeId);
        parent.newNode(nodeId);
    }
    if (node1 == null){
    	nodes[nodeId-100] = node1 = new Node(nodeId-100);
        parent.newNode(nodeId-100);
    }
    if (node2 == null){      
        nodes[nodeId-120] = node2 = new Node(nodeId-120);
        parent.newNode(nodeId-120);
    }
    node.update(messageId, TempData);
    node1.update(messageId, HumidityData);
    node2.update(messageId, PhotoData);
    }
